...
const chaosContainer = document.getElementById('chaos-icons');
let mouseX = -1000;
let mouseY = -1000;

const icons = CHAOS_ICONS.map((icon, i) => {
...
function animateChaos(time) {
  ...
  icons.forEach((icon) => {
    ...
    icon.el.style.transform = `translate(${left}px, ${top}px) rotate(${icon.rotation}deg) scale(${scale})`;
  });
...
billingToggle.addEventListener('click', () => {
  ...
  if (isYearly) {
    ...
    proPeriod.textContent = '/mo (billed $72/yr)';
  } else {
